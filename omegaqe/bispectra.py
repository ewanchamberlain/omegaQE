...
class Bispectra:
    ...
    def _triangle_cross_product(self, mag1, mag2, mag3):
        s = (mag1 + mag2 + mag3) / 2
        res = -2 * np.sqrt(
            s * (s - mag1) * (s - mag2) * (s - mag3)
        )  # I think sign here is arbitrary?
        # res[np.isnan(res)] = 0   # tmp
        return res
    ...
    def _build_M_spline(
        ...
        if ells_sample is not None and M_matrix is not None:
            spline = self._mode.spline(
                ells_sample,
                M_matrix,
                typ=typ,
                gal_distro=gal_distro,
                sec_order_var=sec_var,
            )
    ...
    def build_M_spline(
        ...
        self._M_splines[typ] = self._build_M_spline(
            typ, ells_sample, M_matrix, zmin, zmax, nu, gal_bins
        )
        ...
        for sec_var in typs:
            if sec_var != "k":
                ...
                if M_spline_ld is not None:
                    if (
                        (M_spline_ld.nu == nu)
                        and (M_spline_ld.gal_bins == gal_bins)
                        and (M_spline_ld.gal_distro == gal_distro)
                    ):
                        continue
                print(
                    f"Building M-spline for {sec_var} lens delta matrices for {M_typ1}."
                )
                self._M_splines_lens_delta[sec_var][M_typ1] = self._build_M_spline(
                    M_typ1,
                    None,
                    None,
                    0,
                ...
                if M_typ2 != M_typ1:
                    print(
                        f"Building M-spline for {sec_var} lens delta matrices for {M_typ2}."
                    )
                    self._M_splines_lens_delta[sec_var][M_typ2] = self._build_M_spline(
                        M_typ2,
                        None,
                        None,
                        0,
    ...
    def _build_M_splines(self, typ, nu, gal_bins, gal_distro="LSST_gold"):
        ...
        if self._M_splines[M_typ1] is not None:
            if (
                self._M_splines[M_typ1].nu != nu
                or self._M_splines[M_typ1].gal_bins != gal_bins
                or self._M_splines[M_typ1].gal_distro != gal_distro
            ):
                self._M_splines[M_typ1] = self._build_M_spline(
                    M_typ1, None, None, 0, None, nu, gal_bins, gal_distro=gal_distro
                )
                if M_typ2 != M_typ1:
                    self._M_splines[M_typ2] = self._build_M_spline(
                        M_typ2, None, None, 0, None, nu, gal_bins, gal_distro=gal_distro
                    )
            return
        self._M_splines[M_typ1] = self._build_M_spline(
            M_typ1, None, None, 0, None, nu, gal_bins, gal_distro=gal_distro
    ...
    def _pb_bispectrum(
        self, typ, L1, L2, L3, M_spline, zmin, zmax, nu, gal_bins, gal_distro="LSST_gold"
    ):
        M1, M2, L12_dot = self._bispectra_prep(
            typ,
            L1,
            L2,
            L3,
        ...
        if typ[-1] == "w":
            ...
            raise ValueError(
                f"Bispectrum type {typ} has second order variable {typ[-1]} not from expected 'k' or 'w'."
            )
    ...
    def _omega_bispectrum_angle(
        ...
        return np.sin(2 * theta12) * (
            M1 - M2
        )  # Using clockwise omega convention of Pratten & Lewis
    ...
    def _get_matter_ps(self, L, chi, extended=False):
        ...
        w[k < 1e-4] = 0
        w[k >= 100] = 0
        return w * self._mode._cosmo.get_matter_ps(
            self._mode.matter_PK, z, k, curly=False, weyl_scaled=False, typ="matter"
        )
    ...
    def get_lss_bispectrum(
        ...
        if "w" in typ:
            return 0
        if L3 is None:
            return self.get_lss_bispectrum(
                typ,
                L1,
                L2,
                self._get_third_L(L1, L2, theta),
                None,
                zmin,
                zmax,
                nu,
        ...
        Nchi = 100
        _, chis, dChi, win1, win2 = self._mode._integral_prep(
            Nchi,
            zmin,
            zmax,
            typ[:-1],
        ...
        return (
            np.sum(
                win1 * win2 * win3 * self._delta_bispectrum(L1, L2, L3, chis) / (chis**4),
                axis=-1,
            )
            * dChi
        )

    def _lens_delta_bispectrum(
        self,
        typ,
        L1,
        L2,
    ...
    def _get_lens_delta_bis(
        ...
        if "kk" in "".join(sorted(typ)):
            if typ[0] == "k":  # kak
                ...
                return self._lens_delta_bispectrum(
                    "kk" + typ[1],
                    L1,
                    L3,
                    L2,
        ...
        if "k" in typ:
            if verbose:
                print(
                    f"Including lensed delta {'k' + typ[1] + typ[0]} and {typ[0] + 'k' + typ[1]} bispectra"
                )
            lens_bi1 = self._lens_delta_bispectrum(
                "k" + typ[1] + typ[0],
                L3,
                L2,
                L1,
            ...
            lens_bi2 = self._lens_delta_bispectrum(
                typ[0] + "k" + typ[1],
                L1,
                L3,
                L2,
        ...
        if verbose:
            print(
                f"Including lensed delta {typ} and {typ[0] + typ[2] + typ[1]} and {typ[2] + typ[1] + typ[0]} bispectra"
            )
        lens_bi1 = self._lens_delta_bispectrum(
            typ, L1, L2, L3, M_spline, zmin, zmax, nu, gal_bins, gal_distro=gal_distro
        )
        lens_bi2 = self._lens_delta_bispectrum(
            typ[0] + typ[2] + typ[1],
            L1,
        ...
        lens_bi3 = self._lens_delta_bispectrum(
            typ[2] + typ[1] + typ[0],
            L3,
            L2,
            L1,
    ...
    def get_ld_bispectrum(
        ...
        return self._get_lens_delta_bis(
            typ, L1, L2, L3, M_spline, zmin, zmax, nu, gal_bins, gal_distro, verbose
        )
    ...
    def get_ll_bispectrum(
        self, typ, L1, L2, L3=None, theta=None, M_spline=False, zmin=0, zmax=None
    ):
        if L3 is None:
    ...
    def get_pb_bispectrum(
        ...
        if L3 is not None:
            b = self._pb_bispectrum(
                typ, L1, L2, L3, M_spline, zmin, zmax, nu, gal_bins, gal_distro=gal_distro
            )
            if one_perm:
                if verbose:
                    print(f"Including one permutation of pB bispectrun {typ}")
                return b
            if typ == "kkk":
                if verbose:
                    print(f"Including all three pB bispectra of {typ}")
                b += self._pb_bispectrum(
                    typ,
                    L1,
                    L3,
                ...
                if typ[0] == "k":  # kak
                    if verbose:
                        print(f"Including all two pB bispectra of {typ} (kak)")
                    b += self._pb_bispectrum(
                        typ,
                        L3,
                        L2,
                    ...
                    if verbose:
                        print(f"Including all two pB bispectra of {typ} (akk)")
                    b += self._pb_bispectrum(
                        typ,
                        L1,
                        L3,
                ...
                if verbose:
                    print(f"Including all one pB bispectra of {typ} (abk)")
            return b
        if typ[-1] == "w":
            return self._omega_bispectrum_angle(
                typ,
                L1,
                L2,
                theta,
    ...
    def get_bispectrum(
        ...
        if sec_var in ("w", "k"):
            if verbose:
                print("Including post born bispectra terms")
            b += self.get_pb_bispectrum(
                typ,
                L1,
                L2,
                L3,
            ...
            if verbose:
                print("Including lens-lens post born bispectra terms")
            typ = typ.replace(sec_var, "k")
            b += self.get_ll_bispectrum(typ, L1, L2, L3, theta, M_spline, zmin, zmax)
        elif sec_var == "D":
            if verbose:
                print("Including ray-deflect post born bispectra terms")
            typ = typ.replace(sec_var, "k")
        ...
        if lens_delta:
            ...
            b += self._get_lens_delta_bis(
                typ, L1, L2, L3, M_spline, zmin, zmax, nu, gal_bins, gal_distro, verbose
            )
        if include_lss:
            if verbose:
                print("Including lss bispectrum term")
            b += self.get_lss_bispectrum(
                typ, L1, L2, L3, None, zmin, zmax, nu, gal_bins, gal_distro
            )
        return b
