...
def _BB_mixed(Ls, verbose, N_L1, N_L2, Ntheta1, Ntheta2):
    L1s, thetas1, dTheta1, L2s, thetas2, dTheta2 = _integral_prep(
        N_L1, N_L2, Ntheta1, Ntheta2
    )
    ...
    for iii, L1 in enumerate(L1s):
        ...
        for jjj, theta1 in enumerate(thetas1):
            ...
            I_fac = (
                (L1_vec @ L1p_vec)
                * np.sin(2 * L1_vec.deltaphi(Ls_vec)) ** 2
                * cl_e_spline(L1)
            )
            ...
            for kkk, L2 in enumerate(L2s):
                ...
                L_fac = (
                    (L1p**4 * L2**2) + (2 * L1p**3 * L2**3) + (L1p**2 * L2**4)
                )  # Depending on convention for omega...
                bi = (
                    8
                    * fish.bi.get_bispectrum(
                        "kkw", L1p, L2, theta=theta1p2, M_spline=True
                    )
                    / (L_fac)
                )  # Lfac may be -ve
                I_theta2 = (
                    ...
                    * w3
                    * (L1_vec @ L2_vec)
                    * (L1 * L2p * np.sin(L2p_vec.deltaphi(L1_vec)))
                    * bi
                )

                I_L2[kkk] = InterpolatedUnivariateSpline(thetas2, I_theta2).integral(
                    0, 2 * np.pi - dTheta2
                )
            I_theta1[jjj] = InterpolatedUnivariateSpline(
                L2s, L2s * I_L2 * I_fac
            ).integral(Lmin, Lmax)
        I_L1[iii] = InterpolatedUnivariateSpline(thetas1, I_theta1).integral(
            0, 2 * np.pi - dTheta1
        )
    return (-2 / (2 * np.pi) ** 4) * InterpolatedUnivariateSpline(
        L1s, L1s * I_L1
    ).integral(Lmin, Lmax)
