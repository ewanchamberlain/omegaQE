...
class Reconstruction:
    def __init__(
        ...
        self.isocov = li.get_isocov(
            exp_conf, LDres, HD_res=HDres, pyFFTWthreads=n_threads
        )
        self.curl = None
        self.phi = None
    ...
    def _ell_check(self):
        ellmax_lim = 5000
        lensit_lmax = self.maps.lib_datalm.ell_mat()[2**self.LDres // 2][
            2**self.LDres // 2
        ]
        if lensit_lmax > ellmax_lim:
            warnings.warn(
                f"LDres and HDres setting produce ells > {ellmax_lim}. (lensit maps have ells up to {lensit_lmax})"
            )
    ...
    def _get_exp_noise(self, exp, Lmax=None):
        ...
        return (
            self._deconstruct_noise_curve("TT", exp, 0),
            self._deconstruct_noise_curve("EE", exp, 0),
            0,
        )
    ...
    def _inverse_nonzero(self, array):
        ...
        ret[np.where(array != 0.0)] = 1.0 / array[np.where(array != 0.0)]
        return ret

    def Tmap(self, include_noise=True, sim=0, phi_idx=None, gauss=False):
        if gauss:
            return self.fields.get_cmb_map(
                "TT", include_noise, fft=False, muK=True, sim=sim
            ) / (2 * np.pi)
        map = self.maps.get_sim_tmap(sim, phi_idx) - self.maps.get_noise_sim_tmap(sim)
        if include_noise:
            return map + self.fields.get_noise_map(
                "TT", True, fft=False, muK=True, sim=sim
            ) / (2 * np.pi)
        return map
    ...
    def QUmap(self, include_noise=True, sim=0, phi_idx=None, gauss=False):
        ...
        Qmap = self.maps.get_sim_qumap(sim, phi_idx)[0] - self.maps.get_noise_sim_qmap(
            sim
        )
        Umap = self.maps.get_sim_qumap(sim, phi_idx)[1] - self.maps.get_noise_sim_umap(
            sim
        )
        if include_noise:
            Enoise_fft = self.fields.get_noise_map(
                "EE", True, fft=True, muK=True, sim=sim
            )
            Bnoise_fft = self.fields.get_noise_map(
                "BB", True, fft=True, muK=True, sim=sim
            )
    ...
    def _get_iblm(
        self, fields, include_noise, sim, phi_idx, return_data_alms=False, gaussCMB=False
    ):
        if fields == "T":
            estimator = "T"
            T_alm = self.isocov.lib_datalm.map2alm(
                self.Tmap(include_noise, sim, phi_idx, gaussCMB)
            )
            iblm = self.isocov.get_iblms(
                estimator, np.atleast_2d(T_alm), use_cls_len=True
            )[0]
            if return_data_alms:
    ...
    def _iter_starting_point(self, fields, include_noise, sim, phi_idx):
        estimator, iblm, data_alms = self._get_iblm(
            fields, include_noise, sim, phi_idx, True
        )
        alm_no_norm = 0.5 * self.isocov.get_qlms(
            estimator, iblm, self.isocov.lib_skyalm, use_cls_len=True
        )
        N0 = self.isocov.get_N0cls(estimator, self.isocov.lib_skyalm, use_cls_len=True)
        Cl_phi = li.get_fidcls(wrotationCls=True)[0]["pp"][
            : self.isocov.lib_skyalm.ellmax + 1
        ]
        Cl_omega = li.get_fidcls(wrotationCls=True)[0]["oo"][
            : self.isocov.lib_skyalm.ellmax + 1
        ]
        wiener_filter_phi = self._inverse_nonzero(
            self._inverse_nonzero(N0[0]) + self._inverse_nonzero(Cl_phi)
        )
        wiener_filter_omega = self._inverse_nonzero(
            self._inverse_nonzero(N0[1]) + self._inverse_nonzero(Cl_omega)
        )
        alm_norm_phi = self.isocov.lib_skyalm.almxfl(alm_no_norm[0], wiener_filter_phi)
        alm_norm_omega = self.isocov.lib_skyalm.almxfl(
            alm_no_norm[1], wiener_filter_omega
        )
    ...
    def _get_iter_instance(self, estimator, POlm0, datalms):
        ...
        cpp_priors = [
            np.copy(cls_unl["pp"][: LMAX_SKY + 1]),
            np.copy(cls_unl["oo"][: LMAX_SKY + 1]),
        ]

        lib_skyalm = ell_mat.ffs_alm_pyFFTW(
            self.isocov.lib_datalm.ell_mat, filt_func=lambda ell: ell <= LMAX_SKY
        )
        ...
        transf = gauss_beam(beam / 180.0 / 60.0 * np.pi, lmax=LMAX_SKY)  #: fiducial beam

        filt = ffs_ninv_filt_ideal.ffs_ninv_filt(
            self.isocov.lib_datalm, lib_skyalm, cls_unl, transf, nT, nP
        )

        chain_descr = chain_samples.get_isomgchain(
            filt.lib_skyalm.ellmax, filt.lib_datalm.shape, tol=1e-6, iter_max=200
        )
        ...
        iterator = ffs_iterator_cstMF(
            iter_dir,
            estimator,
            filt,
            datalms,
            self.isocov.lib_skyalm,
            POlm0,
            H0s,
            POlm0 * 0.0,
            cpp_priors,
            chain_descr=chain_descr,
            opfilt=opfilt,
            verbose=True,
        )
        return iterator
    ...
    def _get_iter_normalisation(self, alm, idx, sim, phi_idx):
        ...
        alm_input = (
            self.maps.lencmbs.get_sim_plm(phi_idx)
            if idx == 0
            else self.maps.lencmbs.get_sim_olm(sim)
        )
        alm_input = self.isocov.lib_skyalm.udgrade(
            self.maps.lencmbs.lib_skyalm, alm_input
        )
        cl_cross = self.isocov.lib_skyalm.alm2cl(alm, alm_input)
        cl_corr = self._inverse_nonzero(cl_cross) * self.isocov.lib_skyalm.alm2cl(
            alm_input
        )
    ...
    def _QE_iter(self, fields, idx, include_noise, sim, phi_idx, renorm=False):
        estimator, phi_alm_0, omega_alm_0, data_alms = self._iter_starting_point(
            fields, include_noise, sim, phi_idx
        )
        iter_lib = self._get_iter_instance(
            estimator, np.array([phi_alm_0, omega_alm_0]), data_alms
        )
    ...
    def _QE(
        self, fields, idx, include_noise, sim, phi_idx, gaussCMB, diffSims, diffSim_offset
    ):
        print(
            f"Quadratic estimation of type: {fields}, with noise: {include_noise}, phi_idx: {phi_idx}, gauss fields: {gaussCMB}, N1: {diffSims}, diff_offset: {diffSim_offset}"
        )
        if diffSims:
            phi_idx = sim if phi_idx is None else phi_idx
            estimator, iblm = self._get_iblm(
                fields, include_noise, sim, phi_idx, gaussCMB=gaussCMB
            )
            estimator, iblm2 = self._get_iblm(
                fields, include_noise, sim + diffSim_offset, phi_idx, gaussCMB=gaussCMB
            )
            alm_no_norm = (
                0.5
                * self.isocov.get_qlms(
                    estimator,
                    iblm,
                    self.isocov.lib_skyalm,
                    use_cls_len=True,
                    resp_cls=self.resp_cls,
                    iblms2=iblm2,
                )[idx]
            )
            alm_no_norm += (
        ...
        f = self.isocov.get_response(
            estimator,
            self.isocov.lib_skyalm,
            cls_weights=self.resp_cls,
            cls_cmb=self.resp_cls,
        )[idx]
    ...
    def get_phi_rec(
        ...
        qe_func = self._QE
        self.phi = qe_func(
            fields, 0, include_noise, sim, phi_idx, gaussCMB, diffSims, diffSim_offset
        )
        if return_map:
    ...
    def get_curl_rec(
        ...
        qe_func = self._QE
        self.curl = qe_func(
            fields, 1, include_noise, sim, phi_idx, gaussCMB, diffSims, diffSim_offset
        )
        if return_map:
    ...
    def get_phi_input(self, return_map=False, sim=0):
        P_alm = self.isocov.lib_skyalm.udgrade(
            self.maps.lencmbs.lib_skyalm, self.maps.lencmbs.get_sim_plm(sim)
        )
        if return_map:
    ...
    def _get_rfft_map(self, alm):
        ...
        physical_length = np.sqrt(
            np.prod(self.isocov.lib_skyalm.lsides)
        )  # Why is this needed?
