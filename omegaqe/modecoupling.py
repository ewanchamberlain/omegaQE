...
class Modecoupling:
    ...
    def __init__(self, powerspectra=None, matter_PK=None):
        ...
        self.matter_PK = (
            self._cosmo.get_matter_PK(typ="matter") if matter_PK is None else matter_PK
        )
    ...
    def _get_window(self, typ, chi, nu, gal_bins, gal_distro="LSST_gold", chi_max=None):
        cmb_lens_window = self._cosmo.cmb_lens_window_matter(
            chi, self._cosmo.get_chi_star()
        )
        if typ == "k":
            ...
            if self.use_LSST_abcde:
                ...
                gal_window = self._cosmo.gal_window_Chi(
                    chi, zmin=gal_bins[index], zmax=gal_bins[index + 1], typ=gal_distro
                )
            return gal_window
    ...
    def _integral_prep(
        ...
        if sec_order_var in ("k", "w", "r"):
            ...
            win1 = self._get_window(
                sec_order_var, Chis, nu, gal_bins, gal_distro, Chi_max
            )
    ...
    def _get_ps(
        ...
        if typ[1] == "k":
            return self._powerspectra.get_kappa_ps_2source(
                ells, Chis, Chi_source2, recalc_PK=recalc_PK, use_weyl=False
            )
        if typ[1] == "r":
            return self._powerspectra.get_gal_lens_ps_2source(
                ells, Chis, Chi_source2, recalc_PK=recalc_PK, use_weyl=False
            )
        if typ[1] == "s":
            return self._powerspectra.get_gal_lens_kappa_ps(
                ells, Chis, recalc_PK=recalc_PK, use_weyl=False
            )
        if typ[1] == "g":
            return self._powerspectra.get_gal_kappa_ps(
                ells, Chis, recalc_PK=recalc_PK, gal_distro=gal_distro, use_weyl=False
            )
        ...
        if typ[1] == "I":
            return self._powerspectra.get_cib_kappa_ps(
                ells, nu=nu, Chi_source1=Chis, recalc_PK=recalc_PK, use_weyl=False
            )
        if typ[1] == "u":
            return self._powerspectra.get_mu_kappa_ps(
                ells, Chi_source1=Chis, recalc_PK=recalc_PK, use_weyl=False
            )

    def _get_matter_ps(self, zs, ks):
        return self._cosmo.get_matter_ps(
            self.matter_PK, zs, ks, curly=False, weyl_scaled=False, typ="matter"
        )
    ...
    def _components(
        ...
        zs, Chis, dChi, win1, win2 = self._integral_prep(
            Nchi,
            zmin,
            zmax,
            typ,
    ...
    def _matrix(
        ...
        for iii, ell1 in enumerate(ells1):
            M[iii, :] = self._components(
                np.ones(np.size(ells2)) * ell1,
                ells2,
                typ,
                star,
                Nchi,
