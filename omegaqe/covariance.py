...
class Covariance:
    def __init__(self, cosmology=None):
        ...
        self.shot_noise = (
            [2.25, 3.11, 3.09, 2.61, 2.00] if self.use_LSST_abcde else None
        )  # caution that one bin agora is using n=40
        self.mag_bias = False
    ...
    def _get_Cl_gal(
        ...
        ells = np.arange(ellmax + 1)
        if use_bins:
            cl_g = self.power.get_gal_ps(
                ells,
                gal_win_zmin_a=gal_win_zmin_a,
                gal_win_zmax_a=gal_win_zmax_a,
                gal_win_zmin_b=gal_win_zmin_b,
                gal_win_zmax_b=gal_win_zmax_b,
            ...
            cl_g = self.power.get_gal_ps(
                ells, gal_distro=gal_distro, gal_distro_b=gal_distro_b
            )
        if self.mag_bias:
    ...
    def _get_Cl_gal_kappa(
        ...
        ells = np.arange(ellmax + 1)
        if use_bins:
            cl_kg = self.power.get_gal_kappa_ps(
                ells,
                gal_win_zmin=gal_win_zmin,
                gal_win_zmax=gal_win_zmax,
                gal_distro=gal_distro,
            )
            if self.mag_bias:
                raise ValueError("Mag bias not implementated for binned gal powerspectra")
    ...
    def _get_Cl_cib_gal(
        ...
        ells = np.arange(ellmax + 1)
        if use_bins:
            cl_Ig = self.power.get_cib_gal_ps(
                ells,
                nu=nu,
                gal_win_zmin=gal_win_zmin,
                gal_win_zmax=gal_win_zmax,
    ...
    def _get_Cl(
        ...
        if typ == "kk":
            ...
            return self._get_Cl_gal_kappa(
                ellmax, gal_bins[0], gal_bins[1], use_bins, gal_distro=gal_distro
            )
        elif typ == "gg":
            return self._get_Cl_gal(
                ellmax,
                gal_bins[0],
                gal_bins[1],
                gal_bins[2],
            ...
            return self._get_Cl_cib_gal(
                ellmax, nu, gal_bins[0], gal_bins[1], use_bins, gal_distro=gal_distro
            )
        elif typ == "ww":
        ...
        return self._get_Cl(
            ...
            ellmax,
            nu,
            (gal_win_zmin_1, gal_win_zmax_1, gal_win_zmin_2, gal_win_zmax_2),
            use_bins=True,
            gal_distro=gal_distro,
            gal_distro_b=gal_distro_b,
        )
    ...
    def _get_Cov(
        ...
        if typ[0] == "k":
            ...
            N = self.noise.get_gal_shot_N(
                ellmax=ellmax, zmin=gal_win_zmin, zmax=gal_win_zmax
            )
            # N = 1e-100
            cl = self._get_Cl_gal(
                ellmax,
    ...
    def _get_C_inv(self, typs, Lmax, nu, gal_bins, gal_distro="LSST_gold"):
        ...
        typs_no_fI[typs_no_fI == "f"] = (
            "z"  # Replacing 'f' with 'z' for sympy operations as 'ff' is sympy function
        )
    ...
    def _get_rho(
        self, typ, Lmax, nu, gal_bins, use_bins, gal_distro, include_kappa_noise=True
    ):
        a_1 = typ[0]
        a_2 = typ[1]
        typ1 = a_1 + a_1
    ...
    def _get_corr_inv(self, typs, Lmax, nu, gal_bins, use_bins, gal_distro):
        ...
        rhos = [
            self._get_rho(arg, Lmax, nu, gal_bins, use_bins, gal_distro) for arg in args
        ]
    ...
    def _get_total_tracer_corr(self, tracers, Lmax, nu, gal_bins, use_bins, gal_distro):
        ...
        for iii, a_i in enumerate(tracers):
            rho_ik = self._get_rho(
                ...
                use_bins,
                gal_distro,
                include_kappa_noise=False,
            )
            for jjj, a_j in enumerate(tracers):
                rho_jk = self._get_rho(
                    a_j + "k",
                    Lmax,
                    nu,
                    gal_bins,
    ...
    def get_C_inv(
        self, typs, Lmax, nu, gal_bins=(None, None, None, None), gal_distro="LSST_gold"
    ):
        """
    ...
    def get_corr_inv(
        ...
        """
        return self._get_corr_inv(
            np.char.array(list(typs)), Lmax, nu, gal_bins, use_bins, gal_distro
        )

    def get_total_tracer_corr(
        self,
        tracers,
        Lmax,
        nu=353e9,
        ...
        """
        return self._get_total_tracer_corr(
            np.char.array(list(tracers)), Lmax, nu, gal_bins, use_bins, gal_distro
        )
    ...
    def get_corr(
        ...
        """
        return self._get_rho(
            typ, Lmax, nu, gal_bins, use_bins, gal_distro, include_kappa_noise
        )
    ...
    def get_Cov_mat(
        ...
        """
        return self._get_Cov_mat(
            typs, ellmax, nu, gal_bins, use_bins, gal_distro=gal_distro, noise=noise
        )
