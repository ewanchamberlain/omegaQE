...
def _main(exp, typs, params, condition, dir, _id, H0):
    ...
    F_ww = np.empty(my_end - my_start)
    F_lss = np.empty(my_end - my_start)
    ...
    if my_rank == 0:
        ...
        np.save(dir + "/F_ww", F_arr_ww)
        np.save(dir + "/F_lss", F_arr_lss)
        ...
        world_comm.Send([F_ww, MPI.DOUBLE], dest=0, tag=77)
