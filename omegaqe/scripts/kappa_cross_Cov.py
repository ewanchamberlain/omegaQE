...
def kappa_cross_Cov(
    typs, Ls, F_L, NL1s=1000, F_L_tot=None, tracer_Lmin=30, tracer_Lmax=3000, diag=True
):
    ...
    cosmo.b1 = 0
    fish = Fisher(
        exp="ACT", cosmology=cosmo, setup_bispectra=False
    ...
    for iii in np.arange(np.size(typs)):
        for jjj in np.arange(iii, np.size(typs)):
            ...
            C_inv[iii][jjj] = InterpolatedUnivariateSpline(
                np.arange(np.size(C_inv_ij)), C_inv_ij
            )
        ...
        for jjj in np.arange(iii, jjj_max):
            ...
            for ip in typ_combs:
                ...
                for jq in typ_combs:
                    ...
                    bi_ij = fish.bi.get_bispectrum(
                        f"{i}{j}k", L1s, Lp, L, M_spline=True, one_perm=True
                    )
                    bi_pq = fish.bi.get_bispectrum(
                        f"{p}{q}k", L1s, L, Lp, M_spline=True, one_perm=True
                    )
                    for ur in typ_combs:
                        u = ur[0]
                        r = ur[1]
                        ...
                        Cov_ij = np.sum(
                            bi_ij * bi_pq * cinv_ip * cinv_jr * cinv_qu * c_rk * c_uk
                        )
                        Cov_ij *= (
                            L_facs
                            * cl_kappa_pB[iii]
    ...
    for iii, L in enumerate(Ls):
        ...
        Cov[iii, iii] += (
            (cov_kk * cl_kappa_pB[iii] ** 2 / F_L[iii]) + cl_kappa_cross[iii] ** 2
        ) / ((2 * L) + 1)
    return Ls, Cov
...
def main(typs, Ls, F_L, NL1s, F_L_tot):
    ...
    plt.imshow(
        np.abs(Cov / std_matrix),
        norm=LogNorm(1e-2, 1e0),
        extent=(Ls[0], Ls[-1], Ls[-1], Ls[0]),
    )
...
if __name__ == "__main__":
    ...
    F_L_Ls = np.load(
        f"../../omegaqeNBs/_kappa_F_L_results/{typs}/ACT/gmv/TEB/30_3000/1_2000/Ls_k_pB.npy"
    )
    F_L_tmp = np.load(
        f"../../omegaqeNBs/_kappa_F_L_results/{typs}/ACT/gmv/TEB/30_3000/1_2000/F_L_k_pB.npy"
    )
    F_L_tot_tmp = np.load(
        f"../../omegaqeNBs/_old_kappa_F_L/_kappa_F_L_results_1perm_nob/{typs}/ACT/gmv/TEB/30_3000/1_2000/F_L_k.npy"
    )
