...
def main(path_to_lensit):
    cols = np.loadtxt(
        f"{path_to_lensit}/lensit/data/cls/fiducial_flatsky_lenspotentialCls.dat"
    ).transpose()
    ells = cols[0]
    ...
    col = np.loadtxt(
        f"{path_to_lensit}/lensit/data/cls/fiducial_fieldrotationCls.dat"
    ).transpose()
    ells = np.arange(np.size(col))
    # col = _get_Cl_oo(ells)
    col *= 1e3
    np.savetxt(
        "my_fiducial_fieldrotationCls.dat", col.transpose(), fmt="%.5e"
    )  # File to be added to /lensit/data/cls/
