...
def get_qe_key(fields, gmv):
    if not gmv:
        if fields != "TT":
            raise ValueError(
                f"Cannot get iterative N0 for single qe field pair {fields}. Only TT."
            )
        return "ptt"
    elif gmv:
        if fields == "EB":
            ...
            raise ValueError(
                f"Cannot get iterative N0 for minimum variance combination of {fields}. Only EB or TEB."
            )
...
def get_N_dict(exp, delta_T, beam, Lmax):
    ...
    for fields in ["TT", "EE", "BB"]:
        N = (
            cov.noise.get_cmb_gaussian_N(
                fields, deltaT=delta_T, beam=beam, ellmax=Lmax, exp=exp
            )
            * (2.7255 * 1e6) ** 2
        )
...
def main(exp, fields, gmv, iters, T_Lmin, T_Lmax, P_Lmin, P_Lmax, ext):
    ...
    ellmax = 5000
    cls_unl = {
        key: fac * cov.power.cosmo.get_unlens_ps(key.upper(), ellmax=ellmax)
        for key in keys
    }
    ...
    N0_iter = n0s.get_N0_iter(
        qe_key,
        delta_T,
        delta_P,
        beam,
        cls_unl,
        {"t": T_Lmin, "e": P_Lmin, "b": P_Lmin},
        {"t": T_Lmax, "e": P_Lmax, "b": P_Lmax},
        lmax_qlm=ellmax,
        itermax=iters,
        ret_delcls=False,
        ret_curl=True,
        rho_sqd_ext=rho_sqd_ext,
    )
...
if __name__ == "__main__":
    ...
    if len(args) != 9:
        raise ValueError(
            "Must supply arguments: exp fields gmv iters T_Lmin T_Lmax P_Lmin P_Lmax ext"
        )
    exp = str(args[0])
    fields = str(args[1])
