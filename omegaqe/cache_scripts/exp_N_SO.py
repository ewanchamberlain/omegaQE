...
def get_N(deproj, N_file, offest):
    df = pd.read_csv(N_file, sep=r"\s+", header=None)
    col = int(deproj[-1]) + 1
...
def save(N, typ):
    sep = tools.getFileSep()
    folder = cache_dir + "_N0" + sep + f"SO_{sensitivity}" + sep + "exp"
...
def main(sensitivity):
    ...
    N_file = rf"{data_dir}{sep}SO_noise{sep}exp_noise{sep}SO_LAT_Nell_T_atmv1_{key2}_fsky0p4_ILC_CMB.txt"
    N = get_N(key1, N_file, 40)
    N[2:40] = N[40]  # Modifying noise at Ls 10 -> 39, setting equal to N at L of 40
    save(N, "TT")
    N_file = rf"{data_dir}{sep}SO_noise{sep}exp_noise{sep}SO_LAT_Nell_P_{key2}_fsky0p4_ILC_CMB_E.txt"
    N = get_N(key1, N_file, 10)
    N[2:10] = N[10]  # Modifying noise at Ls 2 -> 10, setting equal to N at L of 10
    save(N, "EE")
    N_file = rf"{data_dir}{sep}SO_noise{sep}exp_noise{sep}SO_LAT_Nell_p_{key2}_fsky0p4_ILC_CMB_B.txt"
    N = get_N(key1, N_file, 10)
    N[2:10] = N[10]  # Modifying noise at Ls 2 -> 10, setting equal to N at L of 10
    save(N, "BB")
