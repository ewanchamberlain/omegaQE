...
def save(N, typ):
    folder = cache_dir + "_N0" + sep + "S4_base" + sep + "exp"
...
def main():
    ...
    N_file = rf"{data_dir}{sep}S4_noise{sep}exp_nooise{sep}S4_190604d_2LAT_T_default_noisecurves_{key1}_SENS0_mask_16000_ell_TT_yy.txt"
    N = get_N(N_file, 1, 40)
    N[2:40] = N[40]  # Modifying noise at Ls 10 -> 39, setting equal to N at L of 40
    save(N, "TT")
    N_file = rf"{data_dir}{sep}S4_noise{sep}exp_nooise{sep}S4_190604d_2LAT_pol_default_noisecurves_{key1}_SENS0_mask_16000_ell_EE_BB.txt"
    N = get_N(N_file, 1, 10)
