...
def get_N0(key1, key2, key3, field):
    print(
        f"Getting lensing Noise + foreground from SO experiment with projection={key1}, sensitivity={key2} and method={key3}"
    )
    N0_file = rf"{data_dir}/SO_noise/nlkk_v3_1_0_{key1}_{key2}_fsky0p4_{key3}_lT30-3000_lP30-5000.dat"
    dict = {
        "TT": (1, 8),
        "TE": (2, 9),
        "EE": (3, 10),
    ...
    df = pd.read_csv(N0_file, sep=r"\s+", header=None)
...
def main(sensitivity):
    ...
    for field in fields_single:
        ...
        folder = cache_dir + "_N0" + sep + f"SO_{sensitivity}" + sep + "single"
    ...
    for field in fields_gmv:
        N0 = get_N0(key1, key2, key3, field)
        folder = cache_dir + "_N0" + sep + f"SO_{sensitivity}" + sep + "gmv"
