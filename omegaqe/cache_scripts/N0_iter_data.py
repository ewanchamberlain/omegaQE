...
def save_N0(exps, powerspectra, fields, convert=False):
    ...
    for exp in exps:
        for ps in powerspectra:
            df_phi = pd.read_csv(
                f"{data_dir}{sep}N0{sep}{exp}{sep}N0_phi_{ps}_T30-3000_P30-5000.csv",
                sep=" ",
            )
            df_curl = pd.read_csv(
                f"{data_dir}{sep}N0{sep}{exp}{sep}N0_curl_{ps}_T30-3000_P30-5000.csv",
                sep=" ",
            )
            for iii, col in enumerate(columns):
                for typ in ["iter", "iter_ext"]:
                    ...
                    N0_phi, N0_curl = np.load(
                        f"{cache_dir}{sep}_N0{sep}{exp}{sep}{typ}{sep}N0_{fields}_T30-3000_P30-5000.npy"
                    )
                    col_name = col + "_" + typ
                    if convert:
                        Ls = np.arange(2, 5001)
            ...
            df_phi.to_csv(
                f"{dir}{sep}N0_phi_{ps}_T30-3000_P30-5000.csv",
                sep=" ",
                float_format="{:,.6e}".format,
            )
            df_curl.set_index("Ls", inplace=True)
            df_curl.to_csv(
                f"{dir}{sep}N0_curl_{ps}_T30-3000_P30-5000.csv",
                sep=" ",
                float_format="{:,.6e}".format,
            )
