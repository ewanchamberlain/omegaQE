...
def _get_cached_F_L(F_L_path, typs, iter, L_min_cut=30, L_max_cut=3000, magbias_typ=None):
    sep = getFileSep()
    ...
    full_F_L_path = (
        ...
        + fields
        + sep
        + f"{L_min_cut}_{L_max_cut}"
        + sep
        + "1_2000"
    )
    magbias_str = f"_u{magbias_typ}" if magbias_typ is not None else ""
    sample_Ls = np.load(full_F_L_path + sep + f"Ls{magbias_str}.npy")
    F_L = np.load(full_F_L_path + sep + f"F_L{magbias_str}.npy")
...
def _get_N2_innerloop(
    ...
    g_XY = global_qe.weight_function(
        XY,
        L_vec,
        l_vec,
        curl=curl,
    ...
    h_X_A1 = global_qe.geo_fac(
        XY[0], theta12=l_primprim_vec.deltaphi(l_vec)
    )  # this is different to N32 paper
    h_Y_A1 = global_qe.geo_fac(
        XY[1], theta12=l_primprim_vec.deltaphi(l_prim_vec)
    )  # ^^^ same ^^^
    ...
    I_C1_theta1 = (
        ...
        * dThetal
        * dl
        * np.sum(bi * ls[None, :] * w_prim * L_C1_fac * C_Ybar_l * g_XY * h_Y_C1)
    )
    if not gmv:
        g_YX = global_qe.weight_function(
            XY[::-1],
            L_vec,
            l_vec,
            curl=curl,
        ...
        I_C1_theta1 += (
            ...
            * dThetal
            * dl
            * np.sum(bi * ls[None, :] * w_prim * L_C1_fac * (C_Xbar_l * g_YX * h_X_C1))
        )
    return I_A1_theta1 + I_C1_theta1


def _get_N1_innerloop(
    XY,
    curl,
    gmv,
    fields,
    ...
    response_ps = "lensed" if iter else "gradient"
    g_XY = global_qe.weight_function(
        XY, L_vec, l_vec, curl=True, gmv=gmv, fields=fields, apply_Lcuts=True
    )
    includeBB = False
    XprimsYprims = (
        global_qe.parse_fields(fields, unique=False, includeBB=includeBB)
        if gmv
        else [fields]
    )  # include BB here?
    ...
    for XprimYprim in XprimsYprims:
        ...
        g_XprimYprim = global_qe.weight_function(
            XprimYprim,
            L1_vec,
            l_vec,
            curl=False,
...
def _get_N0_innerloop(
    ...
    inner_sum = np.zeros(np.shape(w_prim))
    for XpYp in XpYps:
        g_XpYp = global_qe.weight_function(
            XpYp,
            L1_vec,
            l_vec,
            curl=False,
        ...
        for XppYpp in XpYps:
            g_XppYpp = global_qe.weight_function(
                XppYpp,
                L2_vec,
                l_prim_vec,
                curl=False,
    ...
    return (
        ...
        * dThetal
        * dl
        * np.sum(beta * g_XY * inner_sum * ls[None, :] * w_prim * w_primprim)
    )
...
def _bias_calc(
    ...
    for iii, L1 in enumerate(L1s):
        if verbose:
            print(f"    L1 = {L1} ({iii}/{np.size(L1s) - 1})")
        ...
        I_L1[iii] = (
            L1
            * 2
            * InterpolatedUnivariateSpline(thetas1, I_theta1).integral(0, np.pi - dTheta1)
        )
    N = InterpolatedUnivariateSpline(L1s, I_L1).integral(Lmin_lss, Lmax_lss) / (
        (2 * np.pi) ** 4
    )
    return N
...
def _setup_norms():
    ...
    return InterpolatedUnivariateSpline(sample_Ls_w, N0_w), InterpolatedUnivariateSpline(
        sample_Ls_k, N0_k
    )
        ...
        for XY in XYs:
            ...
            if gmv:
                N_tmp = _bias_calc(
                    ...
                    bi_typ,
                    curl,
                    *_bias_prep(fields, True, N_L1, N_L3, Ntheta12, Ntheta13),
                    verbose=verbose,
                    noise=noise,
                    iter=iter,
                )
                ...
                N_tmp = _bias_calc(
                    ...
                    bi_typ,
                    curl,
                    *_bias_prep(XY, False, N_L1, N_L3, Ntheta12, Ntheta13),
                    verbose=verbose,
                    noise=noise,
                    iter=iter,
                )
...
def _get_third_L(L1, L2, theta):
    return np.sqrt(L1**2 + L2**2 + (2 * L1 * L2 * np.cos(theta).astype("double"))).astype(
        "double"
    )
...
def _mixed_bispectrum(typs, L1, L2, theta12, nu):
    ...
    for iii in np.arange(Ncombos):
        bi_ij = global_fish.bi.get_bispectrum(
            combos[iii] + "w", L1, L2, theta=theta12, M_spline=True, nu=nu
        )
...
def _alpha(typs, L1, L2, theta12, nu, no_k=False):
    ...
    for iii in np.arange(Ncombos):
        ...
        for q in typs:
            if (
                q == "k" and no_k is True
            ):  # Get rid of this if not calculating N1 kk terms separately
                continue
            kq = "k" + q  # TODO: should q="k" term be halved?
            cov_inv1_spline, cov_inv2_spline = _get_cov_inv_spline(combos[iii] + kq, typs)
            Cl_qk_spline = _get_Cl_spline(q + "k")
            mixed_bi_element = (
                bi_ij * cov_inv1_spline(L1) * cov_inv2_spline(L2) * Cl_qk_spline(L2)
            )
            # if q == "k":
            # mixed_bi_element /= 2
            if mixed_bi is None:
...
def mixed_bispectrum(bias_typ, bi_typ, L1, L2, theta12, nu=353e9):
    ...
    if bias_typ == "N2":
        if bi_typ == "theory":
            return (
                4
                * global_fish.bi.get_bispectrum(
                    "kkw", L1, L2, theta=theta12, M_spline=True
                )
                / (L1**2 * L2**2)
            )
    ...
    if bias_typ == "N32":
        ...
        return (
            4
            * global_fish.bi.get_bispectrum("kkk", L1, L2, L, M_spline=True)
            / (L1**2 * L2**2)
        )
    if bias_typ == "N1":
        if bi_typ == "theory":
    ...
    if bias_typ == "N0":
        if bi_typ == "theory":
            return global_fish.bi.get_bispectrum(
                "kkw", L1, L2, theta=theta12, M_spline=True
            )
...
def _cache_lss_cls(lss_cls, iter):
    ...
    Cl_kk = (
        global_fish.covariance.get_Cl("kk", ellmax=ellmax)
        if lss_cls is None
        else lss_cls["kk"]
    )
    ...
    Cl_gk = (
        global_fish.covariance.get_Cl("kg", ellmax=ellmax)
        if lss_cls is None
        else lss_cls["gk"]
    )
    ...
    Cl_Ik = (
        global_fish.covariance.get_Cl("kI", ellmax=ellmax)
        if lss_cls is None
        else lss_cls["Ik"]
    )
...
def _setup_delen_cmb_cls():
    ...
    for iii, typ in enumerate(typs):
        ...
        global_qe.cmb[typ].lenCl_spline = InterpolatedUnivariateSpline(
            Ls[2:], delen_len_cl[2:]
        )
        global_qe.cmb[typ].gradCl_spline = InterpolatedUnivariateSpline(
            Ls[2:], delen_len_cl[2:]
        )

        if typ[0] != typ[1]:
            global_qe.cmb[typ[::-1]].lenCl_spline = InterpolatedUnivariateSpline(
                Ls[2:], delen_len_cl[2:]
            )
            global_qe.cmb[typ[::-1]].gradCl_spline = InterpolatedUnivariateSpline(
                Ls[2:], delen_len_cl[2:]
            )


def _cache_splines(F_L_path, bi_typ, lss_cls, iter, magbias_typ):
    global F_L_spline, C_inv_splines
    sample_Ls, F_L = _get_cached_F_L(F_L_path, bi_typ, iter, magbias_typ=magbias_typ)
    F_L_spline = InterpolatedUnivariateSpline(sample_Ls, F_L)
    C_inv = global_fish.covariance.get_C_inv(
        bi_typ, Lmax=int(np.ceil(np.max(sample_Ls))), nu=353e9
    )
...
def bias(
    ...
    global_fish = Fisher(
        exp, qe_fields, gmv, ps, L_cuts, iter, False, data_dir, setup_bispectra=True
    )

    if magbias_typ is not None:
        global_fish.covariance.mag_bias = True
        global_fish.covariance.power.cosmo.set_magbias(magbias_typ)
    ...
    if bi_typ != "theory":
        if verbose:
            print("Caching lss splines")
        _cache_splines(F_L_path, bi_typ, lss_cls, iter, magbias_typ)
