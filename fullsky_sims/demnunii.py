...
class Demnunii:
    ...
    def get_particle_snap(self, snap, lensed=False):
        ...
        if lensed:
            filename = (
                f"{self.cache_dir}{sep}_len_snaps{sep}len_snap_{snap_num_app}{snap}.fits"
            )
        else:
    ...
    def get_snaps_z(self, zmin, zmax):
        ...
        snaps = snaps[indices]
        print(
            f"Using particles between redshifts zmin: {z_starts[indices][0]} (snap {np.max(snaps)}) and zmax: {z_ends[indices][-1]} (snap {np.min(snaps)})"
        )
        return snaps

    def get_density_map(self, zmin=0, zmax=1100, pixel_corr=True, verbose=False):
        if verbose:
            print(f"DEMNUnii: Constructing density map for zmin={zmin}, zmax={zmax}")
        ...
        for iii, snap in enumerate(snaps):
            if verbose:
                print(
                    f"    [{str(datetime.datetime.now() - t0)[:-7]}] Snap: {snap} ({iii + 1}/{np.size(snaps)})",
                    end="",
                )
        ...
        if pixel_corr:
            density_map = self._apply_pixel_correction(density_map)
        return density_map
    ...
    def _window_LSST_mu(self, z):
        ...
        return self.cosmo.gal_lens_window_matter(
            self.cosmo.z_to_Chi(z), self.cosmo.get_chi_star()
        ) / self.cosmo.get_hubble(z)

    def _window_LSST_rsd(self, z):
        # Need to divide by H(z) to convert window into function of z and not chi
        return self.cosmo.rsd_window_constChi(
            self.cosmo.z_to_Chi(z), self.Lmax_map
        ) / self.cosmo.get_hubble(z)
    ...
    def _window_CMB(self, z):
        # Need to divide by H(z) to convert window into function of z and not chi
        return self.cosmo.cmb_lens_window_matter(
            self.cosmo.z_to_Chi(z), self.cosmo.get_chi_star()
        ) / self.cosmo.get_hubble(z)
    ...
    def _window_gal_kappa(self, snap, source_snap):
        ...
        dz = zs[1] - zs[0]
        wins = self.cosmo.cmb_lens_window_matter(
            self.cosmo.z_to_Chi(zs), self._get_chi(source_snap)
        ) / self.cosmo.get_hubble(zs)
    ...
    def _window(self, snap, typ, gal_zmin=None, gal_zmax=None, bias_unity=False):
        ...
        if typ == "CMB":
            ...
            return np.sum(wins) * dz
        raise ValueError(
            "Window type not of 'LSST' (gal), 'Planck' (CIB), or 'CMB' (CMB lensing)."
        )
    ...
    def get_obs_gal_map(
        ...
        if verbose:
            print(
                f"DEMNUnii: Constructing gal map for zmin={zmin}, zmax={zmax}, window={window}, pix_cor={pixel_corr}, lensed={lensed}"
            )
        npix = self.sht.nside2npix()
        gal = np.zeros(npix)
        snaps = self.get_snaps_z(zmin, zmax)
        ...
        for iii, snap in enumerate(snaps):
            ...
            gal += self._window(
                snap, window, zmin, zmax, bias_unity=bias_unity
            ) * self.get_density_snap(snap, lensed)
            if verbose:
                print("\r", end="")
        if verbose:
            print("")
        if pixel_corr:
            gal = self._apply_pixel_correction(gal)
        return gal

    def get_obs_mag_bias_map(
        self, zmin=0, zmax=1100, verbose=False, pixel_corr=True, s_typ=1
    ):
        # TODO: what to do with s... (could use estimated euclid s 1506.01369)
        window = "LSST_mu"
        if verbose:
            print(
                f"DEMNUnii: Constructing mag bias map for zmin={zmin}, zmax={zmax}, window={window}, pix_cor={pixel_corr}"
            )
        npix = self.sht.nside2npix()
        gal = np.zeros(npix)
        snaps = self.get_snaps_z(zmin, zmax)
        ...
        for iii, snap in enumerate(snaps):
            ...
            gal += (
                ((5 * s_spline(self._get_z(snap))) - 2)
                * self._window(snap, window)
                * self.get_density_snap(snap, lensed=False)
            )
            if verbose:
                print("\r", end="")
        if verbose:
    ...
    def get_obs_rsd_map(
        self, zmin=0, zmax=20, verbose=False, pixel_corr=True, lensed=False
    ):
        # TODO: see 2309.00052
        window = "LSST_rsd"
        if verbose:
            print(
                f"DEMNUnii: Constructing rsd map for zmin={zmin}, zmax={zmax}, window={window}, pix_cor={pixel_corr}, lensed={lensed}"
            )
        npix = self.sht.nside2npix()
        gal = np.zeros(npix)
        snaps = self.get_snaps_z(zmin, zmax)
    ...
    def get_obs_cib_map(
        self, zmin=0, zmax=1100, verbose=False, pixel_corr=True, lensed=False
    ):
        window = "Planck_cib"
        if verbose:
            print(
                f"DEMNUnii: Constructing CIB map for zmin={zmin}, zmax={zmax}, window={window}, pix_cor={pixel_corr}, lensed={lensed}"
            )
        npix = self.sht.nside2npix()
        cib = np.zeros(npix)
        snaps = self.get_snaps_z(zmin, zmax)
        ...
        if verbose:
            print("")
        if pixel_corr:
            cib = self._apply_pixel_correction(cib)
        return cib
    ...
    def get_gal_kappa_map(self, source_snap, verbose=False, pixel_corr=True):
        ...
        if verbose:
            print(
                f"DEMNUnii: Constructing galaxy kappa map for zmin={zmin} and zmax={zmax}"
            )
        npix = self.sht.nside2npix()
        ...
        print(
            f"    Source snap: {source_snap}, so using snaps {snaps[-1]} -> {snaps[0]} (zmax: {self._get_zmax(snaps[0])}) to construct kappa."
        )
        t0 = datetime.datetime.now()
        for iii, snap in enumerate(snaps):
            if verbose:
        ...
        if verbose:
            print("")
        if pixel_corr:
            gal_kappa = self._apply_pixel_correction(gal_kappa)
        return gal_kappa

    def get_gal_kappa_snap(self, snap, source_snap, pixel_corr=False):
        gal_kappa = self._window_gal_kappa(snap, source_snap) * self.get_density_snap(
            snap
        )
        if pixel_corr:
            gal_kappa = self._apply_pixel_correction(gal_kappa)
    ...
    def get_kappa_map_z(self, zmin=0, zmax=1100, verbose=False, pixel_corr=True):
        ...
        if verbose:
            print(
                f"DEMNUnii: Constructing kappa map for zmin={zmin}, zmax={zmax}, window={window}"
            )
        npix = self.sht.nside2npix()
        kappa = np.zeros(npix)
        snaps = self.get_snaps_z(zmin, zmax)
        ...
        if verbose:
            print("")
        if pixel_corr:
            kappa = self._apply_pixel_correction(kappa)
        return kappa
    ...
    def get_kappa_map(self, pb=False):
        sep = getFileSep()
        kappa_file = (
            "map0_kappa_ecp262_dmn2_lmax8000.fits"
            if pb
            else "map0_kappa_ecp262_dmn2_lmax8000_first.fits"
        )
    ...
    def get_PK(self, typ="total"):
        ...
        def _get_k(snap):
            ...
            ks_ = np.loadtxt(
                f"{self.data_dir}/powerspectra_LCDM_2Gp_2048_PlTab5_binned_II/ps{typ_name}_{zeros}{snap}.txt",
                usecols=0,
                skiprows=1,
            )
            return ks_ * h
        ...
        for iii, snap in enumerate(snaps[::-1]):
            ...
            ps[iii, np.logical_and(ks > kmin, ks < kmax)] = InterpolatedUnivariateSpline(
                ks_, ps_
            )(ks_overlap)
