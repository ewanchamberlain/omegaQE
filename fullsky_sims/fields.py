...
class Fields:
    def __init__(
        ...
        self.fish = Fisher(
            ...
            False,
            False,
            data_dir=self.nbody.omegaqe_data,
            cosmology=self.nbody.cosmo,
        )
        self.u_typ = u_typ
        if "u" in fields:
        ...
        self._initialise(
            use_lss_cache, use_cmb_cache, gauss_lss, len_lss, use_gauss_chache
        )

    def _initialise(
        self, use_lss_cache, use_cmb_cache, gauss_lss, len_lss, use_gauss_cache
    ):
        self.fft_maps = dict.fromkeys(self._fields)
        self.fft_noise_maps = dict.fromkeys(self._fields)
        for field in self._fields:
            self.fft_maps[field] = self.get_map(
                field, fft=True, use_cache=use_lss_cache, lensed=len_lss
            )
            self.fft_noise_maps[field] = self.get_noise_map(
                field, set_seed=True, fft=True
            )
        if gauss_lss:
            if not use_gauss_cache:
                print("Using Cls of previous maps to generate new gaussian realisations.")
                self.y = self._get_y(
                    input_kappa_map=self.sht.read_map(
                        f"{self.nbody.sims_dir}/kappa_diff_{self.sim}.fits"
                    )
                )
                for field in self._fields:
                    self.fft_maps[field] = self.get_map(
                        field, fft=True, use_cache=use_lss_cache, gaussian=gauss_lss
                    )
            else:
                print(
                    f"Using cached gaussian realisations stored at {self.nbody.sims_dir}/{self.deflect_typ}"
                )
                # fields = self.nbody.sht.read_map(f"{self.nbody.sims_dir}/{self.deflect_typ}/{self.fields}_{self.sim}.fits")
                fields = self.nbody.sht.read_map(
                    f"{self.nbody.sims_dir}/{self.deflect_typ}/kgI_{self.sim}.fits"
                )
    ...
    def setup_rec(self, sim, deflect_typ, iter=False, noise=True, gmv=True, bh=None):
        ...
        self.deflect_typ = deflect_typ
        print(
            f"Creating Reconstruction instance with exp: {self.exp}, and file: {self.nbody.sims_dir}/{self.deflect_typ}/TQU_{self.sim}.fits"
        )
        self.rec = Reconstruction(
            self.exp,
            self.nbody,
            filename=f"{self.nbody.sims_dir}/{self.deflect_typ}/TQU_{self.sim}.fits",
            sim=self.sim,
    ...
    def get_cached_cmb_lens_rec(
        ...
        if bh is not None:
            ext += f"_bh{bh}"
        print(
            f"Getting cached {typ} reconstruction for sim: {sim}, deflection type: {deflect_typ}, exp: {self.exp}, iter: {iter}, gmv: {gmv}, and bias_hard: {bh}"
        )
        return self.sht.read_map(
            f"{self.nbody.sims_dir}/{deflect_typ}/{self.exp}/{typ}/{qe_typ}_{sim}_{ext}.fits"
        )
    ...
    def get_cached_lss(self, field, gaussian, lensed=False):
        ...
        if lensed and field != "k":
            if field == "u":
                ...
                return self.sht.read_map(
                    f"{self.nbody.cache_dir}_maps/u_{self.u_typ}.fits"
                ) + self.sht.read_map(f"{self.nbody.cache_dir}_maps/g_len.fits")
            if field == "r":
                print(f"  Using cached lensed {field} and g maps.")
                return self.sht.read_map(
                    f"{self.nbody.cache_dir}_maps/r.fits"
                ) + self.sht.read_map(f"{self.nbody.cache_dir}_maps/g_len.fits")
    ...
    def _get_cmb_lens_rec_iter(self, typ, cmb_fields):
        ...
        rec_func = (
            self.rec.get_phi_rec_iter if typ == "kappa" else self.rec.get_curl_rec_iter
        )
        return rec_func(itmax)

    def _get_cmb_lens_rec(self, typ, cmb_fields, iter, fft, gmv, cmb_noise, bh=None):
        if self.rec is None:
            map = self.get_cached_cmb_lens_rec(
                typ, cmb_fields, iter=iter, gmv=gmv, cmb_noise=cmb_noise, bh=bh
            )
            if fft:
    ...
    def get_kappa_rec(
        self, cmb_fields="T", iter=False, fft=False, gmv=True, cmb_noise=True, bh=None
    ):
        return self._get_cmb_lens_rec("kappa", cmb_fields, iter, fft, gmv, cmb_noise, bh)

    def get_omega_rec(
        self, cmb_fields="T", iter=False, fft=False, gmv=True, cmb_noise=True, bh=None
    ):
        return self._get_cmb_lens_rec("omega", cmb_fields, iter, fft, gmv, cmb_noise, bh)
    ...
    def _get_N(self, field):
        kmax = 5000
        if field == "k":
            return self.fish.covariance.noise.get_N0(
                "kappa", ellmax=kmax, recalc_N0=False
            )
        if field in ("g", "u", "r"):
        ...
        return self.fish.covariance.noise.get_cmb_gaussian_N(
    ...
    def omega_template(
        ...
        self.tem = Template(
            ...
            use_kappa_rec,
            kappa_rec_qe_typ,
            neg_tracers=neg_tracers,
            iter_mc_corr=iter_mc_corr,
            gmv=gmv,
            bh=bh,
            cmb_noise=cmb_noise,
        )
