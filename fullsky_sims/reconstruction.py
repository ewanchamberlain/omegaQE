...
import shutil
from delensalot.core.opfilt.MAP_opfilt_iso_p import (
    alm_filter_nlev_wl as alm_filter_nlev_wl_pol_only,
)
from delensalot.core.opfilt.MAP_opfilt_iso_tp import (
    alm_filter_nlev_wl as alm_filter_nlev_wl_teb,
)
from delensalot.core.opfilt.MAP_opfilt_iso_t import (
    alm_filter_nlev_wl as alm_filter_nlev_wl_t_only,
)
...
class Reconstruction:
    class MyMapLib:
        ...
        def hashdict(self):
            return {
                "cmbs": self.maps_filename,
                "noise": self.maps_filename,
                "data": self.maps_filename,
            }
    ...
    def __init__(
        ...
        self.nbody = (
            fullsky_sims.wrapper_class(nbody, nthreads)
            if isinstance(nbody, str)
            else nbody
        )
        ...
        if filename is not None:
            self.setup_reconstruction(
                self.filename, seed=sim, iter=iter, noise=noise, gmv=gmv, bh=bh
            )
    ...
    def setup_env(self, exp, iter, sim, filename):
        ...
        self.temp = os.path.join(
            ...
            "demnunii",
            f"{exp}_{iter}",
            filename.replace(f"{self.nbody.sims_dir}/", "")[:-5] + "_" + str(sim),
    ...
    def get_noise_cls(self, exp):
        ...
        return {
            idx[0]: self.noise.get_cmb_gaussian_N(
                idx.upper(), nT, beam, ellmax=self.Lmax_map, exp=exp
            )
            * fac
    ...
    def _apply_cuts(self, filts, indices):
        for iii, filt in enumerate(filts):
            filt[: self._get_Lmin(indices[iii])] *= 0.0
    ...
    def get_filters(self, indices, cl_dict, noise_cls=None):
        ...
        filts = [
            utils.cli(cl_dict[idx][: Lmax_filt + 1] + noise_cls[idx[0]][: Lmax_filt + 1])
            if noise_cls is not None
            else utils.cli(cl_dict[idx][: Lmax_filt + 1])
            for idx in indices
        ]
    ...
    def _raise_typ_error(self, typ):
        raise ValueError(
            f"QE type {typ} not recognized. Recognized types included TEB (for mv), EB (for pol only), or T."
        )
    ...
    def setup_reconstruction(
        self, TQUmaps_filename, seed=None, iter=False, noise=True, gmv=False, bh=None
    ):
        ...
        # cl_wf_nobb['bb'] *=0.
        pix_phas = phas.lib_phas(
            libdir_pixphas, 3, self.Lmax_map, get_state_func=rng_state
        )
        noise_cls = (
            self.noise_cls
            if noise
            else {
                idx[0]: np.zeros(np.size(self.noise_cls[idx[0]])) for idx in self.indices
            }
        )
        self.maps_lib = maps.cmb_maps_harmonicspace(
            self.MyMapLib(self.Lmax_map, TQUmaps_filename, self.sht),
            self.transfer_dict,
            noise_cls,
            noise_phas=pix_phas,
        )
        ...
        if gmv:
            ...
            weighted_maps_lib = filt_simple.library_fullsky_alms_jTP(
            ...
            self.qlms_lib = qest.library_jtTP(
                os.path.join(self.temp, "qlms_dd"),
                weighted_maps_lib,
                weighted_maps_lib,
                self.nbody.nside,
                lmax_qlm=self.Lmax_map,
                resplib=self.qresp_lib,
            )
        else:
            ...
            self.qlms_lib = qest.library_sepTP(
                os.path.join(self.temp, "qlms_dd"),
                weighted_maps_lib,
                weighted_maps_lib,
                cl_wf["te"],
        ...
        self.rdn0_lib = nhl.nhl_lib_simple(
            os.path.join(self.temp, "rdn0"), weighted_maps_lib, cl_wf, self.Lmax_map
        )
        self.setup = True
    ...
    def get_itlib(self, typ, cg_tol: float, libdir_iterator, chain_descrs):
        ...
        Rpp_unl, Roo_unl = qresp.get_response(
            qe_key,
            self.Lmax_map,
            "p",
            self.cl_unl,
        ...
        if typ == "EB":
            filtr = alm_filter_nlev_wl_pol_only(
                nlev_p=noise_e,
                ffi=ffi,
                transf=transfers[1],
                unlalm_info=(self.Lmax_map, self.Lmax_map),
                lenalm_info=(self.Lmax_map, self.Lmax_map),
                transf_b=transfers[2],
                nlev_b=noise_b,
            )
            elm, blm = self.maps_lib.get_sim_pmap(simidx)
            datmaps = np.array(
                [
                    alm_copy(elm, None, self.Lmax_map, self.Lmax_map),
                    alm_copy(blm, None, self.Lmax_map, self.Lmax_map),
                ]
            )
            wflm0 = lambda: alm_copy(
                self.rdn0_lib.ivfs.get_sim_emliklm(simidx),
                None,
                self.Lmax_map,
                self.Lmax_map,
            )
        elif typ == "TEB":
            filtr = alm_filter_nlev_wl_teb(
                nlev_t=noise_t,
                nlev_p=noise_e,
                ffi=ffi,
                transf=transfers[0],
            ...
            datmaps = np.array(
                [
                    alm_copy(tlm, None, self.Lmax_map, self.Lmax_map),
                    alm_copy(elm, None, self.Lmax_map, self.Lmax_map),
                    alm_copy(blm, None, self.Lmax_map, self.Lmax_map),
                ]
            )
            wflm0 = lambda: np.array(
                [
                    alm_copy(
                        self.rdn0_lib.ivfs.get_sim_tmliklm(simidx),
                        None,
                        self.Lmax_map,
                        self.Lmax_map,
                    ),
            ...
            filtr = alm_filter_nlev_wl_t_only(
                nlev_t=noise_t,
                ffi=ffi,
                transf=transfers[0],
                unlalm_info=(self.Lmax_map, self.Lmax_map),
        ...
        iterator = iterator_cstmf(
            libdir_iterator,
            "p",
            [(self.Lmax_map, self.Lmax_map), (self.Lmax_map, self.Lmax_map)],
            datmaps,
            [plm0, olm0],
            ...
            filtr,
            k_geom,
            chain_descrs(self.Lmax_map, cg_tol),
            stepper,
            wflm0=wflm0,
        )
        return iterator
    ...
    def _check_setup(self):
        if not self.setup:
            raise ValueError(
                "Need to call 'setup_reconstruction' first. No qlm_lib has been instantiated."
            )

    def _setup_iter_libdir(self, qe_key):
        TEMP = opj(self.temp, "iter_lib")
        version = "mathew"
        libdir_iterators = lambda qe_key, simidx, version: opj(
            TEMP, "%s_sim%04d" % (qe_key, simidx) + version
        )
    ...
    def calc_iter_rec(self, typ="EB", itmax=15):
        ...
        if itmax >= 0 and Rec.maxiterdone(lib_dir_iterator) < itmax:
            itlib = self.get_itlib(typ, 1.0, lib_dir_iterator, chain_descrs)
    ...
    def get_phi_rec_iter(self, iter):
        if self.iter_rec_data is None:
            raise ValueError(
                "No iterative reconstruction found, run calc_iter_rec first."
            )
    ...
    def get_wiener(self, typ, qe_typ, iter):
        ...
        N0 = self.noise.get_N0(
            typ,
            self.Lmax_map,
            qe=qe_typ,
            gmv=True,
            exp=self.exp,
            T_Lmin=self.L_cuts[0],
            T_Lmax=self.L_cuts[1],
            P_Lmin=self.L_cuts[2],
            P_Lmax=self.L_cuts[3],
            iter=iter,
            recalc_N0=True,
        )
        return Cl / (Cl + N0)
    ...
    def _get_Cl_curl(self, use_cache=True):
        if use_cache:
            ells = np.load("../cache/_C_omega/Ls.npy")[1:]
            Cl_omega = np.load("../cache/_C_omega/C_omega.npy")[1:]
        else:
