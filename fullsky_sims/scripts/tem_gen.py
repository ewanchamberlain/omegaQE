...
def main(
    ...
    mpi.output("-------------------------------------", 0, _id)
    mpi.output(
        f"exp: {exp}, fields: {field_typs}, Nchi: {Nchi}, start: {start}, end: {end}, tracer_noise: {tracer_noise}, kappa_rec: {kappa_rec}, kappa_qe_typ: {kappa_qe_typ}, deflect_typ: {deflect_typ}, gauss_lss: {gauss_lss}, len_lss: {len_lss}, nthreads: {nthreads}, iter_mc_corr: {iter_mc_corr}, gmv: {gmv}, bh: {bh}, cmb_noise: {cmb_noise}, nbody: {nbody}, gauss_cache: {gauss_cache}, u_typ: {u_typ}",
        0,
        _id,
    )
    ...
    for sim in range(start, end):
        ...
        fields = Fields(
            ...
            use_cmb_cache=True,
            cmb_sim=sim,
            deflect_typ=deflect_typ,
            gauss_lss=gauss_lss,
            len_lss=len_lss,
            nthreads=nthreads,
            use_gauss_chache=gauss_cache,
            u_typ=u_typ,
        )
        sht = fields.sht
        ...
        for deflect_typ in deflect_typs:
            ...
            omega_tem = fields.omega_template(
                Nchi,
                tracer_noise=tracer_noise,
                use_kappa_rec=kappa_rec_iii,
                kappa_rec_qe_typ=kappa_qe_typ,
            ...
            tem_map = sht.alm2map(omega_tem, nthreads=fields.nthreads)
...
if __name__ == "__main__":
    ...
    if len(args) != 20:
        raise ValueError(
            "Must supply arguments: exp fields Nchi tracer_noise start end kappa_rec kappa_qe_typ deflect_typ gauss_lss gauss_cache len_lss iter_mc_corr gmv bh cmb_noise nbody gauss_cache u_typ nthreads _id"
        )
    exp = str(args[0])
    fields = str(args[1])
    Nchi = int(args[2])
