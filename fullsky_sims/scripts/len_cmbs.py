...
def _raise_deflection_error(deflect_typ):
    raise ValueError(
        f"Deflection type {deflect_typ} not of dem, diff (or pb for gradient only)."
    )
...
def get_glm(nthreads, deflect_typ, cache_diff_loc=None):
    ...
    if deflect_typ == "dem":
        ...
        if cache_diff_loc is not None:
            ...
            kappa_map = wrapper.get_kappa_map(pb=True)
        Cl_kappa = wrapper.sht.map2cl(
            kappa_map, lmax=LMAX_MAP, lmax_out=LMAX_MAP, nthreads=nthreads
        )
...
def get_clm(nthreads, deflect_typ, cache_diff_loc=None):
    ...
    if deflect_typ == "dem":
        ...
        Cl_omega_smooth = wrapper.sht.map2cl(
            omega_map,
            lmax=LMAX_MAP,
            lmax_out=LMAX_MAP,
            nthreads=nthreads,
...
def get_unlensed_cmb_ps():
    ...
    indices = ["TT", "EE", "BB", "TE"]
    return {
        idx.lower(): wrapper.cosmo.get_unlens_ps(idx, ellmax=LMAX_MAP)[: LMAX_MAP + 1]
        * (Tcmb * 1e6) ** 2
        for idx in indices
    }
...
def get_lensed_cmb_ps():
    ...
    indices = ["TT", "EE", "BB", "TE"]
    return {
        idx.lower(): wrapper.cosmo.get_lens_ps(idx, ellmax=LMAX_MAP)[: LMAX_MAP + 1]
        * (Tcmb * 1e6) ** 2
        for idx in indices
    }
...
def _save_unl_cmbs(loc, alms, sim, nthreads):
    ...
    wrapper.sht.write_map(
        f"{directory}{sep}TQU_{sim}.fits", np.array([Tmap, QUmaps[0], QUmaps[1]])
    )
