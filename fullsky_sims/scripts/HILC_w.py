...
observing_efficiency = (
    0.2 * 0.85
)  # Obsering for 1/5th available days and only using 85% of available map
...
def _get_constrained_w(c_inv):
    ...
    _freqs = (
        freqs + [545] if np.size(freqs) != np.size(a) else freqs
    )  # tmp solution to if planck=True
...
def _fg_maps(tsz, ksz, cib, rad, planck, point, cluster):
    ...
    for iii, freq in enumerate(freqs):
        T_fg, Q_fg, U_fg = ag.create_fg_maps(
            freq, tsz, ksz, cib, rad, False, point, cluster
        )
        Ts_fg[iii, :] = T_fg
        Qs_fg[iii, :] = Q_fg
        Us_fg[iii, :] = U_fg
    if planck:
        T_pl, Q_pl, U_pl = ag.create_fg_maps(
            545, tsz, ksz, cib, False, False, point, cluster
        )
        Ts_fg[-1, :] = T_pl
        Qs_fg[-1, :] = Q_pl
        Us_fg[-1, :] = U_pl
...
def _get_N_white(beam, n_lev):
    T_cmb = 2.7255
    return (
        noise.get_cmb_gaussian_N("TT", deltaT=n_lev, beam=beam, ellmax=ag.Lmax_map)
        * (1e6 * T_cmb) ** 2
    )
...
def _noise_cls(exp, planck):
    ...
    if exp == "SO_base":
        ...
        raise ValueError(
            f"Argument exp: {exp} not expected. Try SO_base or SO_goal or S4_base."
        )
    Nfreqs = len(freqs)
    if planck:
        Nfreqs += 1
    ...
    for iii, nu in enumerate(freqs):
        N_Ts[iii, :] = _noise_cl(
            Nred_Ts[iii], T_ell_knee, T_alpha, beams[iii], n_lev_array[iii]
        )
        N_Ps[iii, :] = _noise_cl(
            None, P_ell_knee, P_alpha, beams[iii], n_lev_array[iii] * np.sqrt(2)
        )
    if planck:
...
def main(exp, tsz, ksz, cib, rad, constrained, planck, point, cluster, nthreads, _id):
    mpi.output("-------------------------------------", 0, _id)
    mpi.output(
        f" tsz: {tsz}, ksz: {ksz}, cib: {cib}, rad: {rad}, constrained: {constrained}, planck: {planck}, point: {point}, cluster: {cluster}, nthreads: {nthreads}",
        0,
        _id,
    )
