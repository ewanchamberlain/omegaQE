...
def _get_smoothed_weights(exp, typ, pm, cm):
    ...
    if cm:
        mask_str += "_cluster"
    w = np.load(
        f"{ag.cache_dir}/_HILC_weights_new/{exp}/{mask_str}/weights_95_150_220_{typ}.npy"
    )
    w_shape = np.shape(w)
...
def main(
    ...
    mpi.output("-------------------------------------", 0, _id)
    mpi.output(
        f"start: {start}, end: {end},  deflect_typ: {deflect_typ}, exp: {exp}, tsz: {tsz}, ksz: {ksz}, cib: {cib}, rad: {rad}, gauss: {gauss}, point_mask: {point_mask}, cluster_mask: {cluster_mask}, nthreads: {nthreads}",
        0,
        _id,
    )
    ...
    for iii, freq in enumerate(freqs):
        T_fg, Q_fg, U_fg = ag.create_fg_maps(
            freq, tsz, ksz, cib, rad, gauss, point_mask, cluster_mask
        )
        Ts_fg[iii, :] = T_fg
        Qs_fg[iii, :] = Q_fg
        Us_fg[iii, :] = U_fg
