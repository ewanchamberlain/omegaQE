...
def main(
    ...
    mpi.output("-------------------------------------", 0, _id)
    mpi.output(
        f"start: {start}, end: {end}, nu: {nu},  tsz: {tsz}, ksz: {ksz}, cib: {cib}, rad: {rad}, gauss: {gauss}, nbody: {nbody_label}, nthreads: {nthreads}",
        0,
        _id,
    )
    ...
    for sim in np.arange(start, end):
        ...
        if gauss:
            ...
            T_fg, tracers = nbody.create_gauss_fg_maps(
                nu,
                tsz,
                ksz,
                cib,
            ...
            T_fg, Q_fg, U_fg = nbody.create_fg_maps(
                nu, tsz, ksz, cib, rad, False, point_mask, cluster_mask
            )
            T += T_fg
            Q += Q_fg
            U += U_fg
