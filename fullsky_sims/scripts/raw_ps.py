...
def _get_ps(
    ...
    omega_tem = nbody.sht.read_map(
        f"{nbody.cache_dir}/_tems/{deflect_typ}/{exp}/{tracer_fields}/omega_tem_{0}{tem_ext}.fits"
    )
    omega_rec = nbody.sht.read_map(
        f"{nbody.sims_dir}/{deflect_typ}/{exp}/omega/{qe_typ}_{0}_{ext}.fits"
    )
    # omega_rec = nbody.sht.read_map(f"{nbody.sims_dir}/{deflect_typ}/{exp}/omega/TEB_{0}_nN_gmv.fits")
    if iter_mc_corr:
        mc_corr = _get_iter_mc_corr_w(exp, qe_typ, gmv)
        omega_rec = nbody.sht.alm2map(
            nbody.sht.almxfl(nbody.sht.map2alm(omega_rec), 1 / mc_corr)
        )
    Cl_ww = nbody.sht.map2cl(omega_tem, omega_rec, nthreads=nthreads)
    for sim in range(1, nsims):
        mpi.output(f"  sim: {sim}", 0, _id)
        omega_tem = nbody.sht.read_map(
            f"{nbody.cache_dir}/_tems/{deflect_typ}/{exp}/{tracer_fields}/omega_tem_{sim}{tem_ext}.fits"
        )
        omega_rec = nbody.sht.read_map(
            f"{nbody.sims_dir}/{deflect_typ}/{exp}/omega/{qe_typ}_{sim}_{ext}.fits"
        )
        # omega_rec = nbody.sht.read_map(f"{nbody.sims_dir}/{deflect_typ}/{exp}/omega/TEB_{sim}_nN_gmv.fits")
        if iter_mc_corr:
            omega_rec = nbody.sht.alm2map(
                nbody.sht.almxfl(nbody.sht.map2alm(omega_rec), 1 / mc_corr)
            )
...
def main(
    ...
    mpi.output("-------------------------------------", 0, _id)
    mpi.output(
        f"exp: {exp}, tracer_fields: {tracer_fields}, nsims: {nsims}, deflect_typ: {deflect_typ}, gauss_lss: {gauss_lss}, len_lss: {len_lss}, cmb_noise: {cmb_noise}, gmv: {gmv}, bh: {bh}, nbody: {nbody_name}, u_typ: {u_typ}, nthreads: {nthreads}",
        0,
        _id,
    )
...
if __name__ == "__main__":
    ...
    if len(args) != 17:
        raise ValueError(
            "Must supply arguments: exp tracer_fields tracer_noise kappa_rec qe_typ nsims deflect_typ gauss_lss len_lss iter_mc_corr cmb_noise gmv bh nbody u_typ nthreads _id"
        )
    exp = str(args[0])
