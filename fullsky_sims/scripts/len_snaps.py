...
def save_lens_maps(len_map, snap, dm, verbose=False):
    directory = Path(dm.cache_dir) / "len_snaps"
    if not os.path.isdir(directory):
        os.makedirs(directory)
    fits_out = directory / f"len_snap_{str(snap).zfill(3)}.fits"
    if verbose:
        print(f"Saving to {fits_out}")
    dm.sht.write_map(str(fits_out), len_map)
...
def main(nthreads, verbose=False):
    ...
    for snap in np.arange(63):
        ...
        save_lens_maps(len_map, snap, dm, verbose=verbose)


if __name__ == "__main__":
    args = sys.argv[1:]
    if len(args) not in (1, 2):
        raise ValueError("Arguments should be nthreads, (optional: is_verbose)")
    nthreads = int(args[0])
